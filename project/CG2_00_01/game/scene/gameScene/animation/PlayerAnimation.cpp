...
void PlayerAnimation::GlovalInit()
{
	...
	// 突撃攻撃時アニメーション
	global_->AddValue<int>("DashAttack", "frameNum", 0);
}
...
void PlayerAnimation::Debug_ImGui()
{
	...
	// 突撃攻撃のフレームの追加
	newFrame = global_->GetValue<int>("DashAttack", "frameNum");
	if (ImGui::Button("DashAttack_NewFrame")) {
		newFrame += 1;
		global_->SetValue<int>("DashAttack", "frameNum", newFrame);
		AddGlobalVariables(newFrame);
	}
...
void PlayerAnimation::AddGlobalVariables(int frameNum)
{
	// 現在のコンボを取得
	std::string Attack = "DashAttack";
	// 追加するフレーム何個目かを取得
	std::string num = std::to_string(frameNum);

	// 何フレームアニメーションするか
	global_->AddValue<float>(Attack, "frame" + num, 60.0f);
	// アニメーションのスラープの速度
	global_->AddValue<float>(Attack, "slerpSpeed" + num, 0.3f);

	// 回転用
	global_->AddValue<Vector3>(Attack, "headAngle" + num, Vector3{});
	global_->AddValue<Vector3>(Attack, "bodyAngle" + num, Vector3{});
	global_->AddValue<Vector3>(Attack, "leftShoulderAngle" + num, Vector3{});
	global_->AddValue<Vector3>(Attack, "rightShoulderAngle" + num, Vector3{});
	global_->AddValue<Vector3>(Attack, "leftArmAngle" + num, Vector3{});
	global_->AddValue<Vector3>(Attack, "rightArmAngle" + num, Vector3{});
	global_->AddValue<Vector3>(Attack, "swordAngle" + num, Vector3{});

	// 座標用
	global_->AddValue<Vector3>(Attack, "headPos" + num, Vector3{});
	global_->AddValue<Vector3>(Attack, "bodyPos" + num, Vector3{});
	global_->AddValue<Vector3>(Attack, "leftShoulderPos" + num, Vector3{});
	global_->AddValue<Vector3>(Attack, "rightShoulderPos" + num, Vector3{});
	global_->AddValue<Vector3>(Attack, "leftArmPos" + num, Vector3{});
	global_->AddValue<Vector3>(Attack, "rightArmPos" + num, Vector3{});
	global_->AddValue<Vector3>(Attack, "swordPos" + num, Vector3{});
}
...
void PlayerAnimation::AnimationUpdate(int frameNum)
{
	// 現在のコンボを取得
	std::string Attack = "DashAttack";
	// 追加するフレーム何個目かを取得
	...
	// 回転
	RotationQuaternion(offsetQ, global_->GetValue<Vector3>("PlayerModelOffset", "headAngle"));
	RotationQuaternion(slerpQ, global_->GetValue<Vector3>(Attack, "headAngle" + num));
	playerModels_->GetHeadTrans()->rotation_.Slerp((offsetQ * slerpQ), slerpSpeed);
	// 座標
	slerpV = global_->GetValue<Vector3>(Attack, "headPos" + num) + global_->GetValue<Vector3>("PlayerModelOffset", "head");
	Vector3LerpShortPosition(playerModels_->GetHeadTrans()->translation_, slerpV, slerpSpeed);

	/* ========== 体 ========== */
	// 回転
	RotationQuaternion(offsetQ, global_->GetValue<Vector3>("PlayerModelOffset", "bodyAngle"));
	RotationQuaternion(slerpQ, global_->GetValue<Vector3>(Attack, "bodyAngle" + num));
	playerModels_->GetBodyTrans()->rotation_.Slerp((offsetQ * slerpQ), slerpSpeed);
	// 座標
	slerpV = global_->GetValue<Vector3>(Attack, "bodyPos" + num) + global_->GetValue<Vector3>("PlayerModelOffset", "body");
	Vector3LerpShortPosition(playerModels_->GetBodyTrans()->translation_, slerpV, slerpSpeed);

	/* ========== 左肩 ========== */
	// 回転
	RotationQuaternion(offsetQ, global_->GetValue<Vector3>("PlayerModelOffset", "leftShoulderAngle"));
	RotationQuaternion(slerpQ, global_->GetValue<Vector3>(Attack, "leftShoulderAngle" + num));
	playerModels_->GetLeftShoulderTrans()->rotation_.Slerp((offsetQ * leftShoulder_Quat_ * slerpQ), slerpSpeed);
	// 座標
	slerpV = global_->GetValue<Vector3>(Attack, "leftShoulderPos" + num) + global_->GetValue<Vector3>("PlayerModelOffset", "leftShoulder");
	Vector3LerpShortPosition(playerModels_->GetLeftShoulderTrans()->translation_, slerpV, slerpSpeed);

	/* ========== 右肩 ========== */
	// 回転
	RotationQuaternion(offsetQ, global_->GetValue<Vector3>("PlayerModelOffset", "rightShoulderAngle"));
	RotationQuaternion(slerpQ, global_->GetValue<Vector3>(Attack, "rightShoulderAngle" + num));
	playerModels_->GetRightShoulderTrans()->rotation_.Slerp((offsetQ * rightShoulder_Quat_ * slerpQ), slerpSpeed);
	// 座標
	slerpV = global_->GetValue<Vector3>(Attack, "rightShoulderPos" + num) + global_->GetValue<Vector3>("PlayerModelOffset", "rightShoulder");
	Vector3LerpShortPosition(playerModels_->GetRightShoulderTrans()->translation_, slerpV, slerpSpeed);

	/* ========== 左腕 ========== */
	// 回転
	RotationQuaternion(offsetQ, global_->GetValue<Vector3>("PlayerModelOffset", "leftArmAngle"));
	RotationQuaternion(slerpQ, global_->GetValue<Vector3>(Attack, "leftArmAngle" + num));
	playerModels_->GetLeftArmTrans()->rotation_.Slerp((offsetQ * slerpQ), slerpSpeed);
	// 座標
	slerpV = global_->GetValue<Vector3>(Attack, "leftArmPos" + num) + global_->GetValue<Vector3>("PlayerModelOffset", "leftArm");
	Vector3LerpShortPosition(playerModels_->GetLeftArmTrans()->translation_, slerpV, slerpSpeed);

	/* ========== 右腕 ========== */
	// 回転
	RotationQuaternion(offsetQ, global_->GetValue<Vector3>("PlayerModelOffset", "rightArmAngle"));
	RotationQuaternion(slerpQ, global_->GetValue<Vector3>(Attack, "rightArmAngle" + num));
	playerModels_->GetRightArmTrans()->rotation_.Slerp((offsetQ * slerpQ), slerpSpeed);
	// 座標
	slerpV = global_->GetValue<Vector3>(Attack, "rightArmPos" + num) + global_->GetValue<Vector3>("PlayerModelOffset", "rightArm");
	Vector3LerpShortPosition(playerModels_->GetRightArmTrans()->translation_, slerpV, slerpSpeed);

	/* ========== 剣 ========== */
	// 回転
	RotationQuaternion(offsetQ, global_->GetValue<Vector3>("PlayerModelOffset", "swordAngle"));
	RotationQuaternion(slerpQ, global_->GetValue<Vector3>(Attack, "swordAngle" + num));
	playerModels_->GetSwordTrans()->rotation_.Slerp((offsetQ * slerpQ), slerpSpeed);
	// 座標
	slerpV = global_->GetValue<Vector3>(Attack, "swordPos" + num) + global_->GetValue<Vector3>("PlayerModelOffset", "sword");
	Vector3LerpShortPosition(playerModels_->GetSwordTrans()->translation_, slerpV, slerpSpeed);
...
void PlayerAnimation::DashAttack()
{
	// アニメーションを更新
	...
	// 次のコンボに行ける時間
	nextCombo_ = false;
	if (dashFrame_ > 2.0f && dashFrame_ <= 3.0f) {
		nextCombo_ = true;
	}

	// 最後のフレームまで行ったらフラグを完了にする
	if (dashFrame_ > static_cast<float>(animationFrameNum)) {
		dashCompletion_ = true;
	}
}
